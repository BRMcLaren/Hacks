    ...
    const char** syms)
{
    ...
    _args.num_syms = num_syms;
    _args.syms = (char**)syms;

    /* Compute input buffer size. Include in and in-out parameters. */
    OE_ADD_SIZE(_input_buffer_size, sizeof(test_args_t));
    if (syms)
        OE_ADD_SIZE(_input_buffer_size, (_args.num_syms * sizeof(char*)));
    ...
    if (syms)
        OE_WRITE_IN_PARAM(syms, (_args.num_syms * sizeof(char*)), char**);
    ...
    const char** syms)
{
    ...
    /* Marshalling struct. */
    test_unwind_args_t _args, *_pargs_in = NULL, *_pargs_out = NULL;
    ...
    /* Compute input buffer size. Include in and in-out parameters. */
    OE_ADD_SIZE(_input_buffer_size, sizeof(test_unwind_args_t));
    if (syms)
        OE_ADD_SIZE(_input_buffer_size, (_args.num_syms * sizeof(char*)));
    
    /* Compute output buffer size. Include out and in-out parameters. */
    OE_ADD_SIZE(_output_buffer_size, sizeof(test_unwind_args_t));
    /* There were no corresponding parameters. */
    ...
    /* Serialize buffer inputs (in and in-out parameters). */
    _pargs_in = (test_unwind_args_t*)_input_buffer;
    OE_ADD_SIZE(_input_buffer_offset, sizeof(*_pargs_in));
    if (syms)
        OE_WRITE_IN_PARAM(syms, (_args.num_syms * sizeof(char*)), char**);
    ...
    /* Setup output arg struct pointer. */
    _pargs_out = (test_unwind_args_t*)_output_buffer;
    OE_ADD_SIZE(_output_buffer_offset, sizeof(*_pargs_out));
...
static oe_ocall_func_t __backtrace_ocall_function_table[] = {
    ...
    oe_enclave_t** enclave)
{
    return oe_create_enclave(
               ...
               config_size,
#endif
               __backtrace_ocall_function_table,
               0,
               enclave);
}
